...
header('Content-Disposition: attachment; filename="customer-contacts.csv"');
...
while($row=mysqli_fetch_assoc($query_run_last_saved)){
	$last_saved_contact=$row['customer_contact'];
}
...
if ($userId>$last_saved_contact){
	$user_CSV[$count] = array(ucfirst($row['name']).' Customer '.$storename, ucfirst($row['name']).' Customer '.$storename,'','','', '','','','','','','',
	'','','','','','','',
	'','','','','','','','','','Mobile',$row['phone']);
	$count++;
	$lastuserid=$userId;
...
$query_update_last_saved='UPDATE last_saved_contact SET customer_contact='.$lastuserid;

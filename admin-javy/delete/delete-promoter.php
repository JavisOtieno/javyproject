...
if($row=mysqli_fetch_assoc($query_run)){
	...
	echo '<label>Promoter Id:</label><br/>';
	echo '<div><strong>'.$id.'</strong></div>';
	echo '<div style="margin-top:10px"><label>Storename:</label><br/>';
	echo '<div><strong>'.$row['storename'].'</strong></div>';
	echo '<div style="margin-top:10px"><label>Promoter Name:</label><br/>';
	echo '<div><strong>'.ucfirst($row['firstname']).' '.ucfirst($row['lastname']).'</strong></div>';
    ...
    font-size: 16px;
    margin: 4px 2px;
    cursor: pointer;" onclick="deletePromoter('.$id.')">Delete promoter</button></div>';
...
<script type="text/javascript">
  ...
  xhttp.open("GET", "../submit/promoter-deleted.php?id="+id, true);
  xhttp.send();

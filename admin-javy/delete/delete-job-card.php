...
if($row=mysqli_fetch_assoc($query_run)){
  ...
  echo '<label>Job Card Id:</label><br/>';
  ...
  echo '<div><strong>'.$row["timestamp"].'</strong></div>';
  ...
  echo '<div id="messages"></div>';
    ...
    font-size: 16px;
    margin: 4px 2px;
    cursor: pointer;" onclick="deleteJobCard('.$id.')">Delete job card</button></a></div>';
...
<script type="text/javascript">
  ...
  xhttp.onreadystatechange = function() {
   ...
   var object = JSON.parse(this.responseText);
    ...
    setTimeout(function () {
       window.location.href = "../job-cards.php";
    }, 1000);
  ...
  xhttp.open("GET", "../submit/job-card-deleted.php?id="+id, true);
  xhttp.send();

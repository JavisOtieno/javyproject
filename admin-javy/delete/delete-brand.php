...
if($row=mysqli_fetch_assoc($query_run)){
	...
	echo '<label>Brand Id:</label><br/>';
	echo '<div><strong>'.$id.'</strong></div>';
	echo '<div style="margin-top:10px"><label>Brand Name:</label><br/>';
	echo '<div><strong>'.$row['brand_name'].'</strong></div>';
	echo '<div style="margin-top:10px"><label>Brand Slug:</label><br/>';
	echo '<div><strong>'.$row['brand_slug'].'</strong></div>';
    ...
    font-size: 16px;
    margin: 4px 2px;
    cursor: pointer;" onclick="deleteBrand('.$id.')">Delete brand</button></div>';
...
<script type="text/javascript">
  ...
  xhttp.open("GET", "../submit/brand-deleted.php?id="+id, true);
  xhttp.send();

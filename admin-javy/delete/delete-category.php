...
if($row=mysqli_fetch_assoc($query_run)){
	...
	echo '<label>Category Id:</label><br/>';
	echo '<div><strong>'.$id.'</strong></div>';
	echo '<div style="margin-top:10px"><label>Category Name:</label><br/>';
	echo '<div><strong>'.$row['categories_name'].'</strong></div>';
	echo '<div style="margin-top:10px"><label>Category Slug:</label><br/>';
	echo '<div><strong>'.$row['categories_slug'].'</strong></div>';
    ...
    font-size: 16px;
    margin: 4px 2px;
    cursor: pointer;" onclick="deleteCategory('.$id.')">Delete category</button></a>';
...
<script type="text/javascript">
  ...
  xhttp.open("GET", "../submit/category-deleted.php?id="+id, true);
  xhttp.send();

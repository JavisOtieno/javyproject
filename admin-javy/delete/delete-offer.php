...
if($row=mysqli_fetch_assoc($query_run)){
	...
	echo '<div style="margin-top:10px"><label>Offer title:</label><br/>';
	echo '<div><strong>'.$row['title'].'</strong></div>';
	echo '<div style="margin-top:10px"><label>Offer Image Link:</label><br/>';
    ...
    font-size: 16px;
    margin: 4px 2px;
    cursor: pointer;" onclick="deleteOffer('.$id.')">Delete offer</button></div>';
...
<script type="text/javascript">
  ...
  xhttp.open("GET", "../submit/offer-deleted.php?id="+id, true);
  xhttp.send();

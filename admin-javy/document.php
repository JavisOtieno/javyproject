...
$dealer_id=$row['store_id'];
$GLOBALS['customer_name']=$row['name'];
$GLOBALS['customer_phone']=$row['phone'];
$GLOBALS['delivery_details']=$row['address'];
$deal_date=$row['timestamp'];
...
function Header()
{
    ...
    $this->Cell(0,0,$GLOBALS['store_name'],0,0,'L');
        ...
        $this->Cell(0,0,' ',0,0,'C');
        $this->SetX($this->lMargin);
        $this->Cell( 0, 0, $GLOBALS['title'], 0, 0, 'R' ); 
...
$pdf->Cell(0,7,$GLOBALS['title'].' Number : '.$id,0,1,'R');

$pdf->Ln(5);

$pdf->Cell(0,7,'          TO :  '.$GLOBALS['customer_name'],0,1);
...
$headerTable = array('Quantity', 'Id', $GLOBALS['description'], 'Price','Amount');
$data= array($quantity, $product_or_cart_id ,$product_name, number_format($product_price),number_format($product_price));

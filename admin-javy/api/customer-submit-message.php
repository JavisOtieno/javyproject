...
$customer_id=mysqli_real_escape_string($db_link,$_GET['customer_id']);
...
if($customer_id>0){
  $querydb="SELECT * FROM `customers` WHERE id ='".$customer_id."'";
...
$email!="NONE"){
  ...
  //$dealer_id as 0
  $dealer_id=0;
  $sqlcustomers = "INSERT INTO customers VALUES(NULL,'$name','$phone_number','$email', '$delivery_details','$dealer_id',0,'','$message_date')";
...
mail($to, $subject, $message_email_javis, $headers);
    ...
    $valid['success'] = true;
    $valid['messages'] = "Thank you. Your message has been received and is now being processed. Message number : ".$order_id
    ." Date : ".date('d/m/Y \a\t h:iA' , $orderdate); 
  }else  {
    $valid['success'] = false;
...
if($customer_password==''){
  $customer_password_set=false;
  $valid['password_set']=$customer_password_set;
}else{
  $customer_password_set=true;
  $valid['password_set']=$customer_password_set;
}

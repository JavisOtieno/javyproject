...
if(isset($_GET['id'])){
    ...
    $store['store_name']="Javy. To change your store. Select Account & then change store";
...
if( $row=mysqli_fetch_array($query_run) ){
   ...
   if( $row2=mysqli_fetch_array($query_run2) ){
    ...
    $store['store_id']=$row['dealerid'];
    $store['store_name']=ucfirst($row2['storename']);
    $store['store_phone']=$row2['phone'];
    $store['store_email']=$row2['email'];

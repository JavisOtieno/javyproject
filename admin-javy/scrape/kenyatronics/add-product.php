...
$link = $_GET['link'];
	...
	$html = file_get_html($link);
			...
			$website_url=str_replace('https://www.kenyatronics.com', ' ', $link);
		...
		$specs_from_website = $html->find("div[class='p_desc'] ",0);
		...
		$finalspecs=trim($specs_from_website->plaintext);
			...
			$heading_from_website = $html->find("h1[itemprop='name']",0)->plaintext;
			$heading_from_website = trim($heading_from_website);
			$heading_from_website = str_replace('"', '', $heading_from_website);
			

			if(strpos(strtolower($heading_from_website),'tv') !== false){
				$category_from_heading='tvs';
	...
	<select class="form-control" id="brandName" name="brandName" >
				      	...
				      	$sql = "SELECT brand_id, brand_name, brand_status,brand_slug FROM brands WHERE brand_status = 1 AND brand_category =".$category_id;
								...
								while($row = $result->fetch_array()) {
									...
									if (strpos(strtolower($heading_from_website),$row[3]) !== false){
										$selected='selected';
									}else{$selected='';}
...
<div id="messages"></div>
	...
	<?php 
		...
		/*$image_wrapper = $html->find('.zoom-small-image',0);
			

			//if we need to see all the images uncomment the echo 
			foreach ($image_wrapper->find('a') as $key ) {
				# code...
				//echo "<img src='".$key->attr['src']."'/>";
			}
			
			echo "<img src='".$image_wrapper->find('a',0)->attr['href']."'/>";

			*/
			echo "To code on images later";

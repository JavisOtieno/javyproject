...
$link = $_GET['link'];
			...
			$website_url=str_replace('https://www.ramtons.com', ' ', $link);
    ...
    while($variable_count<$variables_size){
      ...
      //echo $variant_variable.' @ '.$variant_price.'<br>';
			...
			$heading_from_website = strtolower($heading_from_website);
			$heading_from_website = ucwords($heading_from_website);
			...
			$specs_from_website_initial= $html->find('div[class="product attribute description"]',0);
			//echo $specs_from_website_initial->plaintext;
			$specs_from_website= $html->find('div[class="product attribute description"]',0)->find('ul',0);

			//echo $specs_from_website->plaintext;

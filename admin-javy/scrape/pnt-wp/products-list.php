...
echo '<form method="GET" action="products-list.php" id="linkForm">';
    ...
    echo '<div style="display:inline-block;margin:10px;"><a href="products-list.php?link=https://www.phonestablets.co.ke/shop/page/1/?orderby=date"><button>phonestablets</button></a></div>';
...
<div class="btn-group">
  ...
  echo '<a class="dropdown-item" href="products-list.php?link=https://www.phonestablets.co.ke/product-category/'.$category_key.'s/?orderby=date">All '.ucfirst($row_category['categories_name']).'</a>';
...
while($row_brand=mysqli_fetch_assoc($query_run_brands)){
	...
	echo '<a class="dropdown-item" href="products-list.php?link=https://www.phonestablets.co.ke/product-category/'.$category_key.'s/'.$brand_slug.'-'.$category_key.'s/?orderby=date">'.$brand_name.'</a>';
...
$setlink = 'https://www.phonestablets.co.ke/shop/page/1/?orderby=date';
}




echo '<br/><div style="display:inline-block;margin:10px;"><a href="products-list.php?link=https://www.phonestablets.co.ke/shop/page/1/?orderby=date"><button>All Products</button></a></div>';
...
if(strpos($setlink, 'page') !== false){
    $link_array=explode('/', $setlink);
    ...
    $setlinknextpage=str_replace($current_page, $next_page, $setlink);
    $setlinkpreviouspage=str_replace($current_page, $previous_page, $setlink);


} else{

	if(strrpos($setlink, '?orderby=date')!== false){
	$setlinknextpage=str_replace('?orderby=date', 'page/2/?orderby=date', $setlink);
	}else{
    $setlinknextpage=$setlink.'/page/2';
	}
}
echo '<div style="display:inline-block;margin:10px;"><a href="products-list.php?link='.$setlinkpreviouspage.'"><button>Previous Page</button></a></div>';

echo '<div style="display:inline-block;margin:10px;"><a href="products-list.php?link='.$setlinknextpage.'"><button>Next Page</button></a></div><br/>';
...
foreach ($html->find('.product') as $link) {
	...
	/*echo $link->plaintext;
		...
		When Accessories jumbotron was still active


		if(strpos($setlink, 'shop') !== false)
		{
    ...
    echo 'passes here';
		...
		*/
		...
		$product_link=substr($product_link, 0, strlen($product_link)-1);
		...
		$query2="SELECT * FROM products WHERE link LIKE '%".$product_link."%'";
		...
		if(mysqli_num_rows($query_run2)==0){
			...
			$row_product=mysqli_fetch_assoc($query_run2);

			echo "ON SITE";
			if($row_product['status']==0){
				echo " -- NOT AVAILABLE ON SITE";

				$html = file_get_html($product_link);

				if(strpos( $html->plaintext , 'Out of stock' ) === false)
					{

			$strip_ksh = str_replace('KShs', '', $product_price);
			...
			echo '---<a href="../../submit/set-product-as-available.php?id='.$row_product['id'].'&price='.$int_price.'&profit='.$profit.'">SET AS AVAILABLE</a>';
						}

...
if($row=mysqli_fetch_assoc($query_run)){
  ...
  $link=$row['link'];
    ...
    if(empty($description)&&!empty($link)){
      ...
      //change of phonestablets website 

   /* if((strpos($link, 'phonestablets') !== false) ){

    $html = file_get_html($link);

    $description_from_website = utf8_encode($html->find('div[class="mdl-grid"]',0));

if((strpos($description_from_website, 'Phones &amp; Tablets') !== false) || (strpos($description_from_website, 'Phones&amp;Tablets') !== false) || (strpos($description_from_website, 'Phones&Tablets') !== false) || (strpos($description_from_website, 'Phones & Tablets') !== false) ){

    $description_from_website= str_replace('Phones &amp; Tablets', 'our online shop in' , $description_from_website);
    $description_from_website= str_replace('Phones&amp;Tablets', 'our online shop in' , $description_from_website);
    $description_from_website= str_replace('Phones&Tablets', 'our online shop in' , $description_from_website);
    $description_from_website= str_replace('Phones & Tablets', ' ', $description_from_website);
    ...
    $description_from_website= str_replace('https://www.phonestablets.co.ke', '' , $description_from_website);
    ...
    //remove images
     ...
     if (strpos($description_from_website, 'phonestablets') === false) {
      echo '<br>CLEAR<br><br>';
     }else{
...
solid #CCCCCC .75pt;padding:7.0pt 7.0pt 7.0pt 7.0pt;background:whitesmoke"]') as $item) {
  ...
  foreach($description_from_website ->find('div[style="mso-element:para-border-div;border:solid #CCCCCC 1.0pt;mso-border-alt:  solid #CCCCCC .75pt;padding:7.0pt 7.0pt 7.0pt 7.0pt;background:whitesmoke"]') as $item) {
   ...
   // echo $description_from_website->plaintext;
    ...
    //phonestablets change in website
  
    if((strpos($link, 'phonestablets') !== false) ){

      

if((strpos($description_from_website, 'Phones &amp; Tablets') !== false) || (strpos($description_from_website, 'Phones&amp;Tablets') !== false) || (strpos($description_from_website, 'Phones&Tablets') !== false) || (strpos($description_from_website, 'Phones & Tablets') !== false) || (strpos($description_from_website, 'Phones and Tablets Kenya') !== false)  ){

    $description_from_website= str_replace('Phones &amp; Tablets', 'our online shop in' , $description_from_website);
    $description_from_website= str_replace('Phones&amp;Tablets', 'our online shop in' , $description_from_website);
    $description_from_website= str_replace('Phones&Tablets', 'our online shop in' , $description_from_website);
    $description_from_website= str_replace('Phones and Tablets Kenya', 'our online shop in Kenya' , $description_from_website);
    $description_from_website= str_replace('Phones & Tablets', ' ', $description_from_website);
    ...
    $description_from_website= str_replace('https://www.phonestablets.co.ke', '' , $description_from_website);
    $description_from_website= str_replace('https://phonestablets.co.ke', '' , $description_from_website);
    $description_from_website= str_replace('width="1170" height="658"', 'width="90%" height="270"', $description_from_website);
    $description_from_website= str_replace('data-src', 'src', $description_from_website);
...
//phonestablets change in website
  ...
  if((strpos($description_from_website, 'Starmac Technologies') !== false) || (strpos($description_from_website, 'Cellular Kenya') !== false) || (strpos($description_from_website, 'CELLULAR KENYA') !== false)  ){
	...
	echo '<br>';
	echo '<form method="POST" action="../submit/submit-product-description.php" id="editDescriptionForm">';
	echo '<div class="row"><div class="col-md-10"><div style="margin-left:20px;"><div class="form-group"><label>Product Description:</label><br/>';
    echo '<textarea class="form-control" name="description" style="overflow:auto;width:80%;height:300px;">'.$description.'</textarea></div>';
...
$("#editDescriptionForm").unbind('submit').bind('submit',function(){

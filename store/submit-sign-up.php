...
if($_POST) {		
	...
	$email = $connect->real_escape_string($_POST['email3']);
	$email2 = $connect->real_escape_string($_POST['email4']);
	...
	if(  empty($name) || empty($phonenumber) || empty($email) || empty($address) || empty($password) || ($email2!="") || $linkinstring ) {
		...
		if($password == ""){
			...
			$mainSql = "INSERT INTO customers VALUES (NULL,'$name','$phonenumber','$email','$address','$id',0,'$password',$date)";

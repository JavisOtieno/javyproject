...
$query="SELECT * FROM `suppliers` WHERE `id` ='$supplier'";
...
if($_POST) {		
	...
	$phonenumber = $connect->real_escape_string($_POST['phone']);
	...
	if(  empty($name) || empty($phonenumber) || empty($email) || empty($address) || empty($password) || ($email2!="") ) {
		...
		if($email2 != "") {
			...
			$errors[] = "Only fill in the required details";
		} 

		if($address == ""){
			
			$errors[] = "Address/Location details are required";
		}
		...
		if($password == ""){
			...
			$mainSql = "INSERT INTO customers VALUES(NULL,'$name','$phonenumber','$email', '$address',0,'$supplier_id','$password',$sign_up_date)";
		...
		if($password_set=="not-set"){
			...
			if($mainResult = $connect->query($mainSql)) {
				...
				$subject = 'NEW CUSTOMER REGISTRATION on '.$username;
				$message = 'Name: '.$name.'
				'.'Phone: '.$phonenumber.'
				'.'Supplier id: '.$supplier_id.' username/website: '.$username.' name: '.$fullname.'phone: '.'
				'.'Email: '.$email.'
				...
				$email_sendgrid->setFrom("info@javy.co.ke", ucfirst($username));
				$email_sendgrid->setSubject("SIGN UP SUCCESFUL");
				$email_sendgrid->addTo($email, $name);
				$email_sendgrid->addContent("text/plain", "Hello, ".$name.". Thank you for signing up on www.".$username.".av.ke . We're glad to have you as our customer " );
				...
				$email_sendgrid->setSubject("New Customer sign up on www.".$username.".av.ke");
				$email_sendgrid->addTo($supplier_email, $fullname);
				$email_sendgrid->addContent("text/plain", "Hello, ".$fullname.". A new customer signed up on www.".$username.".av.ke. Login to www.javy.co.ke for details" );
...
catch ( AfricasTalkingGatewayException $e )
{
  //echo "Encountered an error while sending: ".$e->getMessage();
	...
	$valid['messages'] = $errors[0];									
		...
		$valid['success'] = true;
	$valid['messages'] = "Sign up Succesful. Redirecting you...";
	}

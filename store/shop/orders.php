...
<?php
    ...
    if($row['status']==2){
      ...
      $label="warning";
   	...
   	echo '<tr class="'.$label.'">
        ...
        echo '<td><a href="order.php?orderId='.$row['id'].'"><button>View Order</button></a></td>';
        if ($status=="processing") {
           echo '<td><a href="cancel-order.php?orderId='.$row['id'].'"><button>Cancel Order</button></a></td>';
        }else{
          echo '<td></td>';
        }
      echo '</tr>';
   ...
   if(mysqli_num_rows($result)==0){
   	echo "<tr class='danger'><td colspan=2>You have not made any orders yet.<td/><tr/>";
   }

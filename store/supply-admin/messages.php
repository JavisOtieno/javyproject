...
$sqlTotalMessages="SELECT * FROM product_messages WHERE supplier_id=$userId";
$result=$connect->query($sqlTotalMessages);
$numberofproductmessages=$result->num_rows;

$sqlAnsweredMessages= "SELECT * FROM suppliers_contact_forms WHERE supplier_id=$userId";
...
<div class="row">
	<div class="col-md-12">
		...
		<ol class="breadcrumb">
		  ...
		  <li><a href="orders.php">Sales</a></li>		  
				...
				?></caption>
					...
					<tbody>
						...
						if($type=="contact"){
							...
							$sqlcontactmessages="SELECT * FROM suppliers_contact_forms WHERE supplier_id='$userId' ORDER BY id DESC";
						$contactResult=$connect->query($sqlcontactmessages);

						if($contactResult->num_rows>0){
							while($row=$contactResult->fetch_assoc()){
								echo "<tr>";
								echo '<td>'.date('d/m/Y \a\t h:iA', $row['time']).'</td>';
								echo '<td>'.$row['name'].'</td>';
								echo '<td>'.$row['message'].'</td>';

								echo '<td><label class="label label-success">Received</label></td>';
						...
						$sqlwithdrawals="SELECT * FROM suppliers_product_messages WHERE supplier_id='$userId' ORDER BY id DESC";
						...
						if($withdrawalsResult->num_rows>0){
							while($row=$withdrawalsResult->fetch_assoc()){
								...
								if($row['status'] == 1) {
								 		...
								 		$messageStatus = "<label class='label label-danger'>Cancelled</label>";
								 	}

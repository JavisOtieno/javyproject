		...
		<div class="container">
			...
			$query='SELECT * FROM `products` WHERE category="'.$category.'" AND status="1" AND (approval="2" OR (approval="1" AND store_id='.$user_id.') OR supplier_id="'.$supplier_id.'" OR id IN (SELECT product_id FROM more_suppliers WHERE supplier_id='.$supplier_id.')) AND brand="'.$brand.'"';
				...
				if($shop_type==0){
					...
					$query='SELECT * FROM `products` WHERE status="1" AND (approval="2" OR (approval="1" AND store_id='.$user_id.') OR id IN (SELECT product_id FROM more_suppliers WHERE supplier_id='.$supplier_id.') OR supplier_id="'.$supplier_id.'")'.$category_slug_query;
				}
			...
			$query='SELECT * FROM `products` WHERE category="'.$category.'" AND status="1" AND (approval="2" OR (approval="1" AND store_id='.$user_id.') OR supplier_id="'.$supplier_id.'" OR id IN (SELECT product_id FROM more_suppliers WHERE supplier_id='.$supplier_id.')) AND brand="'.$brand.'" ORDER BY price LIMIT '.$page1.',20';
				...
				$query=$query_first.' ORDER BY category,brand,price LIMIT '.$page1.',20';
...
<a href="shop.php<?php if(isset($category)){echo '?category='.$category;} if(isset($_GET['brand'])){echo '&brand='.$brand; }?><?php if(isset($category)){echo '&page=';}else {echo '?page=';} ?><?php echo $page-1;  ?>" <?php if($page==1){ echo 'style="display: none;"';} ?>>&laquo; Previous</a>
  ...
  <a href="shop.php<?php if(isset($category)){echo '?category='.$category;} if(isset($_GET['brand'])){echo '&brand='.$brand; } ?><?php if(isset($category)){echo '&page=';}else {echo '?page=';} ?><?php echo $page+1;  ?>" <?php if($page==$pages){ echo 'style="display: none;"';} ?>>&raquo; Next</a>
			...
			<div class="col-md-3 rsidebar">
					...
					<div class="sidebar-row" style="margin-top: 0;">
						...
						<ul class="faq">
							...
							while($row=mysqli_fetch_assoc($query_run_categories)){
								...
								$query_check_other=$query='SELECT * FROM `products` WHERE status="1" AND (approval="2" OR (approval="1" AND store_id='.$user_id.')  OR supplier_id="'.$supplier_id.'" OR id IN (SELECT product_id FROM more_suppliers WHERE supplier_id='.$supplier_id.')) AND brand="other" AND category="'.$category_slug.'"';
				...
				<?php 
				//commenting out irrelevant searches and you may also like for store without electronics
				if($shop_type>1){
					echo "<!--";
				}
				?>
				<div class="related-row">
					<h4>Top Searches</h4>
					<ul>
						<li><a href="product.php?p=xiaomi_redmi_9a">Xiaomi Redmi 9A</a></li>
						<li><a href="product.php?p=tecno_spark_8">Tecno Spark 8</a></li>
						<li><a href="product.php?p=apple_iphone_13_–_128gb">iPhone 13 Pro Max</a></li>
						<li><a href="shop.php?category=phones&brand=iphone">iPhone </a></li>
						<li><a href="search.php?search=samsung">Samsung</a></li>
						<li><a href="search.php?search=sony+tv">Sony TVs</a></li>
					</ul>
				</div>
				...
				$query='SELECT * FROM `products` WHERE approval!=0 AND status=1 AND id=9675';
								$query_run=mysqli_query($db_link,$query);
								$count=1;
				...
				<div class="related-row">
					...
					<div class="galry-like">  
						<a href="product.php?p=<?php echo $slug; ?>"><img src="<?php echo $image_uri; ?>" class="img-responsive" alt="img"></a>             
						<h4><a href="product.php?p=<?php echo $slug; ?>"><?php echo $name; ?></a></h4> 
			...
			if(isset($category)){
				...
				if($shop_type==0){
					...
					$query='SELECT * FROM `products` INNER JOIN `categories` ON products.category = categories.categories_slug WHERE featured="1" AND shop_type="'.$shop_type.'"';
